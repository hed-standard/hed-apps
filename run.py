...
def run(command, env={}):
    with open(os.path.join('/bids_dataset', 'dataset_description.json'), 'r') as fp:
        dataset_description = json.load(fp)
    os.makedirs("/outputs")
    with open(os.path.join("/outputs", "my.json"), 'w') as fp1:
        json.dump(dataset_description, fp1)
    with open(os.path.join("/outputs", "my.txt"), 'w') as fp2:
        fp2.write("Help me")
...
# with open(os.path.join(args.output_dir, "temp.txt"), 'w') as fp:
